...
class Affiliate_Directorist_Pricting_Plan extends Affiliate_WP_Base
{
    ...
    {
        add_action('after_bb_atpp_gifting_plan', array($this, 'iap_add_pending_referral'), 10, 1);
        //add_action('init', array($this, 'atbdp_order_completed_test'));
     ...
     */
    public function iap_add_pending_referral($order_info)
    {
    ...
    /**
     * Pricing Plan On Order Completed
     */

    public function atbdp_order_completed_test()
    {
        file_put_contents(dirname(__FILE__) . '/log.json', json_encode($this->was_referred()));
    }
    ...
    public function atbdp_order_completed($order_id, $listing_id)
    {
        ...
        $this->referral_type = 'sale';

        $referral_total = $this->calculate_referral_amount($order_amount, $order_id);
