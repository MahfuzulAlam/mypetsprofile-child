...
class MEC_feature_fes extends MEC_base
{
    ...
    public function vform($atts = array())
    {
        ...
        if (!is_user_logged_in() and (!isset($this->settings['fes_guest_status']) or (isset($this->settings['fes_guest_status']) and $this->settings['fes_guest_status'] == '0'))) {
            // Show message
            $message = sprintf(__('Please %s/%s in order to submit new events.', 'mec'), '<a href="' . wp_login_url($this->main->get_full_url()) . '">' . esc_html__('Login', 'mec') . '</a>', '<a href="' . wp_registration_url() . '">' . esc_html__('Register', 'mec') . '</a>');
    ...
    public function vlist($atts = array())
    {
        ...
        if (!is_user_logged_in()) {
            // Show message
            $message = sprintf(__('Please %s/%s in order to manage events.', 'mec'), '<a href="' . wp_login_url($this->main->get_full_url()) . '">' . esc_html__('Login', 'mec') . '</a>', '<a href="' . wp_registration_url() . '">' . esc_html__('Register', 'mec') . '</a>');
    ...
    public function mec_fes_csv_export()
    {
        ...
        $columns = array(__('ID', 'mec'), esc_html__('Event', 'mec'), esc_html__('Date', 'mec'), esc_html__('Order Time', 'mec'), $this->main->m('ticket', esc_html__('Ticket', 'mec')), esc_html__('Transaction ID', 'mec'), esc_html__('Total Price', 'mec'), esc_html__('Gateway', 'mec'), esc_html__('Name', 'mec'), esc_html__('Email', 'mec'), esc_html__('Ticket Variation', 'mec'), esc_html__('Confirmation', 'mec'), esc_html__('Verification', 'mec'));
        ...
        foreach ($post_ids as $post_id) {
            ...
            foreach ($attendees as $key => $attendee) {
                ...
                $booking = array(
                    ...
                    $order_time,
                    (isset($tickets[$ticket_id]['name']) ? $tickets[$ticket_id]['name'] : esc_html__('Unknown', 'mec')),
                    $transaction_id,
    ...
    public function fes_form()
    {
        ...
        if ($agreement_status) {
            ...
            if (!$checked) $this->main->response(array('success' => 0, 'message' => __('You should accept the terms and conditions.', 'mec'), 'code' => 'TERMS_CONDITIONS'));
        }
        ...
        elseif ($status == 'publish') $message = esc_html__('The event published.', 'mec');
